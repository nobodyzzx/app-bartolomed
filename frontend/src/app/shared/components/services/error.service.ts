...
export class ErrorService {
  ...
  private getErrorTitle(statusCode: number): string {
    return (
      this.ERROR_MESSAGES.TITLES[statusCode as keyof typeof this.ERROR_MESSAGES.TITLES] ||
  ...
  private showErrorAlert(title: string, message: string): void {
    this.alert.fire({
      icon: 'error',
      title,
      html: `<div style="font-size: 16px; text-align: center; padding: 10px;">${message}</div>`,
