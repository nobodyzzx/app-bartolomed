...
export class AssetMaintenanceService {
    ...
    if (filters) {
      ...
      if (filters.dateFrom) {
        filteredRecords = filteredRecords.filter(
          record => new Date(record.maintenanceDate) >= filters.dateFrom!,
        )
      }
      if (filters.dateTo) {
        filteredRecords = filteredRecords.filter(
          record => new Date(record.maintenanceDate) <= filters.dateTo!,
        )
      }
    }
  ...
  getMaintenanceStats(): Observable<any> {
    const stats = {
      ...
      completed: this.mockMaintenanceRecords.filter(r => r.status === MaintenanceStatus.COMPLETED)
        .length,
      scheduled: this.mockMaintenanceRecords.filter(r => r.status === MaintenanceStatus.SCHEDULED)
        .length,
      inProgress: this.mockMaintenanceRecords.filter(
  ...
  getMaintenanceByStatus(status: MaintenanceStatus): Observable<AssetMaintenance[]> {
    const records = this.mockMaintenanceRecords.filter(r => r.status === status)
    return of(records).pipe(delay(600))
  }

  getMaintenanceByType(type: MaintenanceType): Observable<AssetMaintenance[]> {
    const records = this.mockMaintenanceRecords.filter(r => r.type === type)
    return of(records).pipe(delay(600))
  }
  ...
  schedulePreventiveMaintenance(
    ...
    for (let i = 1; i <= 4; i++) {
      // Programar para los próximos 4 intervalos
